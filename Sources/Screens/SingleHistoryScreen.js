...
export default function SingleHistoryScreen() {
  ...
  console.log(s_historyData.EventMainImage);
  ...
  const renderTicket = () => (
    ...
    >
      <View style={styles(colorScheme).ticketdataView}>
        <View style={styles(colorScheme).ticketBanner}>
          <Image
            ...
            source={{ uri: s_historyData.EventMainImage }}
          />
        </View>
        <View style={{ width: "55%", gap: 10 }}>
          <Text
            style={[
              styles(colorScheme).ticketData,
              {
                fontSize: 24,
            ...
            numberOfLines={1}
          >
            {s_historyData.EventName}
          </Text>
          <Text style={styles(colorScheme).ticketData}>
            {s_historyData.EventDate} || {s_historyData.EventTime}
          </Text>
          <Text style={styles(colorScheme).ticketData}>
            {s_historyData.AddressLandMark} ,{s_historyData.CityName}
          </Text>
          <Text style={styles(colorScheme).ticketData}>
            {s_historyData.MobileNo}
          </Text>
        </View>
      </View>
      ...
      <View
        style={{
          borderWidth: 1,
          borderColor: colorScheme === "dark" ? "#888888" : "#E2E2E2",
          marginVertical: 20,
        }}
      />
      ...
      {s_historyData.SubDetails.map((detail, index) => (
        <View key={detail.BookTicketDeatilsid}>
          ...
          >
            <View style={{ gap: 15 }}>
              <Text
                style={styles(colorScheme).ticketData}
              >{`${detail.BookingTicketQty} Ticket(s)`}</Text>
              <Text
                style={[
                  styles(colorScheme).ticketData,
                  {
              ...
              >{`Available Ticket-Qty :   ${
                detail.AvailableTicketQty - detail.SacnneTicketQty
              }`}</Text>
            </View>
            <View
              style={{
            ...
            >
              <Text
                style={[
                  ...
                  {
                    fontSize: 16,
                    color: detail.Expired === "Yes" ? "#ff3333" : "green",
                  },
                ]}
              >
                {detail.Expired === "Yes" ? "Expired" : "Active"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles(colorScheme).scannedTicketQty}>
            <Text
              style={[
                styles(colorScheme).ticketData,
                {
                  color: colorScheme === "dark" ? "#FFFFFF" : "#464646",
                  fontSize: 16,
                  width: "100%",
                },
              ]}
                  ...
                  {detail.SplitList.map((person, index) => (
                    <View key={index} style={{ alignItems: "center" }}>
                      ...
                      <View style={{ width: "100%" }}>
                        ...
                        >
                          {" "}
                          ({person.BookingTicketQty} Tickets)
                        </Text>
                      </View>
                    </View>
                  ))}
          ...
          {s_historyData.SubDetails.length === index + 1 ? null : (
            ...
            >
              ...
              <View
                style={{
                  borderWidth: 1,
                  borderColor: colorScheme === "dark" ? "#000000" : "#5A5A5A",
                  borderStyle: "dashed",
                  width: "85%",
                  marginHorizontal: 15,
                }}
